...
//
#include <CCameraSensor/UEyeBlockingCamera.h>
...
namespace CCameraSensor {
    ////////////////////////////////////////////////////////////////////////////    
    const int UEyeBlockingCamera::NUM_RING_BUFFER_IMAGES = 10;

    ////////////////////////////////////////////////////////////////////////////    
    ////////////////////////////////////////////////////////////////////////////    
    ////////////////////////////////////////////////////////////////////////////    
    UEyeBlockingCamera::UEyeBlockingCamera() : m_bOpened( false ), m_bFirstRead( true ), 
        ...
        m_pReadImageHolder( NULL ), m_nPrevAcquisitionTime(0) {
        //m_rEventThreadToComputeTimeOffset( this ) { 
        m_mParameters[ "ExternalTrigger" ] = "0";
    }

    ////////////////////////////////////////////////////////////////////////////    
    UEyeBlockingCamera::~UEyeBlockingCamera() {
        close();
        if( m_pCamList != NULL ) {
            delete m_pCamList;
        }
    ...
    ////////////////////////////////////////////////////////////////////////////    
    bool UEyeBlockingCamera::set( const std::string& sKey, const std::string& sParameter ) {
        std::cerr << "ERROR in set, unknown option: " << sKey << "with param: " << sParameter << std::endl;
        return false;
    }

    ////////////////////////////////////////////////////////////////////////////    
    bool UEyeBlockingCamera::open() {
        if( m_bOpened ) {
        ...
        if( is_GetNumberOfCameras( &m_nNumConnectedCams ) == IS_SUCCESS ) {
            ...
            std::cerr << "ERROR: in UEyeBlockingCamera::open(), call to num. of cameras failed." << std::endl;
            return false;
        }

        if( m_nNumConnectedCams <= 0 ) {
            std::cerr << "ERROR: in UEyeBlockingCamera::open(), got " << m_nNumConnectedCams << " cameras." << std::endl;
            return false;
        }
        ...
        if( is_GetCameraList( m_pCamList ) != IS_SUCCESS ) {
            std::cerr << "ERROR: in UEyeBlockingCamera::open(), problem when calling is_GetCameraList." << std::endl;
            return false;
        }
        ...
        if( is_InitCamera( &m_hCamera, NULL ) != IS_SUCCESS ) {            
            std::cerr << "ERROR: in UEyeBlockingCamera::open(), problem starting the driver (is_InitCamera)." << std::endl;
            return false;
        }

        // Get sensor info
        if( is_GetSensorInfo( m_hCamera, &m_SensorInfo ) != IS_SUCCESS ) {
            std::cerr << "ERROR: in UEyeBlockingCamera::open(), problem when calling is_GetSensorInfo." << std::endl;
            return false;
        }
        ...
        if( is_SetImageSize( m_hCamera, m_nImageWidth, m_nImageHeight ) != IS_SUCCESS ) {
            std::cerr << "ERROR: in UEyeBlockingCamera::open(), problem when calling is_SetImageSize." << std::endl;
            return false;
        }
		...
		if( is_SetColorMode( m_hCamera, IS_CM_MONO8 ) != IS_SUCCESS ) {
            std::cerr << "ERROR: in UEyeBlockingCamera::open(), problem when calling is_SetColorMode." << std::endl;
            return false;
        }
        ...
        for( int ii=0; ii<NUM_RING_BUFFER_IMAGES; ii++ ) {
            ...
            if( is_AllocImageMem
                ...
                std::cerr << "ERROR: in UEyeBlockingCamera::open(), problem when calling is_AllocImageMem." << std::endl;
                return false;
            }

            if( is_AddToSequence 
                ( m_hCamera, pImageMem, pid ) != IS_SUCCESS ) {
                std::cerr << "ERROR: in UEyeBlockingCamera::open(), problem when calling is_AddToSequence." << std::endl;
                return false;
            }
        ...
        if( is_GetImageMemPitch( m_hCamera, &m_nImageWidthStep ) != IS_SUCCESS ) {
            std::cerr << "ERROR: in UEyeBlockingCamera::open(), problem when calling is_GetImageMemPitch." << std::endl;
            return false;
        }
        ...
        if( is_SetExposureTime( m_hCamera, IS_SET_ENABLE_AUTO_SHUTTER, &dRealExp ) != IS_SUCCESS ) {
            std::cerr << "ERROR: in UEyeBlockingCamera::open(), problem when calling is_SetExposureTime." << std::endl;
            return false;
        }
        
        // Make sure trigger mode is off
        if( is_SetExternalTrigger( m_hCamera, IS_SET_TRIGGER_OFF ) != IS_SUCCESS ) {
            std::cerr << "ERROR: in UEyeBlockingCamera::open(), problem when calling is_SetExternalTrigger." << std::endl;
            return false;
        }
        ...
        if( !m_rEventThreadToComputeTimeOffset.start( m_hCamera, IS_SET_EVENT_FRAME ) ) {
            std::cerr << "ERROR: in UEyeBlockingCamera::open(), problem starting time offset thread." << std::endl;
            return false;
        }    
#endif
        ...
        if( is_CaptureVideo( m_hCamera, IS_DONT_WAIT ) != IS_SUCCESS ) {
            std::cerr << "ERROR: in UEyeBlockingCamera::open(), problem when calling is_CaptureVideo." << std::endl;
            return false;
        }
    ...
    ////////////////////////////////////////////////////////////////////////////    
    bool UEyeBlockingCamera::close() {
        std::cout << "Closing UEyeBlockingCamera." << std::endl; 
    ...
    ////////////////////////////////////////////////////////////////////////////    
    bool UEyeBlockingCamera::read( std::vector<CCameraImage::Image*>& vImages ) {
        if( !m_bOpened ) { 
            std::cerr << "ERROR: in UEyeBlockingCamera::read(), has open() been called?" << std::endl;
            return false;
        }
        if( m_hCamera == 0 ) {
            std::cerr << "ERROR: in UEyeBlockingCamera::read() incorrect camera identifier, has open() been called?" << std::endl;
            return false;
        }
        ...
        bool bIsNew = false;
        unsigned long long nAcquisitionTime = 0;
        unsigned long long nPCTime = 0;
        while( !bIsNew ) {
            if( is_GetActSeqBuf( m_hCamera, &nDummy, &pDummy, &pLast ) != IS_SUCCESS ) {
                std::cerr << "ERROR: in UEyeBlockingCamera::read(), problem when calling is_GetActSeqBuf." << std::endl;
                return false;
            }
            
            if( pLast == NULL ) {
                std::cerr << "ERROR: in UEyeBlockingCamera::read(), last camera buffer is NULL (call too early?)." << std::endl;
                return false;
            }
            ...
            if( is_GetImageInfo( m_hCamera, nNum,
                ...
                std::cerr << "ERROR: in UEyeBlockingCamera::read(), problem when calling is_GetImageInfo." << std::endl;
                return false;
            }
            nAcquisitionTime = ImageInfo.u64TimestampDevice/10;
            if( nAcquisitionTime != m_nPrevAcquisitionTime ) {
                bIsNew = true;
                m_nPrevAcquisitionTime = nAcquisitionTime;
                nPCTime = ImageInfo.TimestampSystem.wMilliseconds; // FIX ME: need to compute full time from secs, mins, hours, ...
            }
            else {
                usleep( 5000 );
            }
        ...
        m_pReadImageHolder->cameraTime( nAcquisitionTime );
        // Not very good time ~1ms precision        
        m_pReadImageHolder->time( nPCTime );
        ...
        //std::cout << "Acquisition time: " << m_pReadImageHolder->cameraTime() << std::endl;
#if 0
        double dFPS;
        if( is_GetFramesPerSecond( m_hCamera, &dFPS ) != IS_SUCCESS ) {
            std::cerr << "ERROR: in UEyeBlockingCamera::open(), problem when calling is_GetFramesPerSecond." << std::endl;
            return false;
        }
