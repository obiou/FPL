...
%
function estimates = MinSSDSL3MotionMagnBlurAffineIllumination( IRef, ICur, ICurMask, template, estimates, minInfo, debug )
...
if bDisp
  if bHPrev
    TIRef = blur_warping( IRef, HRef*inv(HPrev), lambdaEst*logm(HEst), HPrev, nWidth, nHeight );
  else
    TIRef = blur_warping( IRef, HRef, lambdaEst*logm(HEst), eye(3), nWidth, nHeight );
  end
...
for nIter=1:nMaxIter
  [HEstCur,lambdaEstCur,alphaEstCur,betaEstCur,RMS,dNormUpdate,TIRefEst,TICurEst] = ...
      SSDUpdateSL3MotionMagnBlurAffineIllumination( IRef, ICur, ICurMask, HRef, HPrev, nWidth, nHeight, HEstPrev, lambdaEstPrev, ...
                                                    nNumParametersH, alphaEstPrev, betaEstPrev, bHPrev, bRobust );
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function test( Hi, lambda, nNumParametersH, alpha, beta )
...
if nargin == 0  
  ...
  test( Hi, lambda, nNumParametersH, alpha, beta );
  return
end
...
IRef  = I;
if bHPrev
  [ICur,ICurMask]  = blur_warping( I, HRef*inv(HPrev), lambda*logm( Hi ), inv( Hi )*inv( HRef ), nImageWidth, nImageHeight );
...
estimates = MinSSDSL3MotionMagnBlurAffineIllumination( IRef, ICur, ICurMask, template, estimates, minInfo, debug );
