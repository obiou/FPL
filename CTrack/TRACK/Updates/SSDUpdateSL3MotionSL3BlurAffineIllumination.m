...
%
function [HEst,MEst,alphaEst,betaEst,RMS,dNormUpdate,TIRefEst,TICurEst] = ...
    SSDUpdateSL3MotionSL3BlurAffineIllumination( IRef, ICur, HRef, nWidth, nHeight, HEst, MEst, ...
                                                 nNumParametersH, nNumParametersM, alphaEst, betaEst, bRobust )
...
HEst     = HEst*LieToH( dH );
MEst     = MEst + LieH( dM );
alphaEst = alphaEst + d( nNumParametersH+nNumParametersM+1:nNumParametersH+nNumParametersM+1 );
betaEst  = betaEst  + d( nNumParametersH+nNumParametersM+2:nNumParametersH+nNumParametersM+2 );
...
for ii=1:nMaxIterPrev
  [HEst,alphaEst,betaEst,RMS,dNormUpdate,TICurEst] = SSDUpdateSL3MotionAffineIllumination( TIRef, ICur, HRef, HEst, nNumParametersH, alphaEst, betaEst );
  subplot( 3, 2, 5 )
  imshow( uint8( TIRef ) );  
...
% Full estimation
for ii=1:nMaxIter
  [HEst,MEst,alphaEst,betaEst,RMS,dNormUpdate,TIRefEst,TICurEst] = ...
      SSDUpdateSL3MotionSL3BlurAffineIllumination( IRef, ICur, HRef, nWidth, nHeight, HEst, MEst, ...
                                                   nNumParametersH, nNumParametersM, alphaEst, betaEst );
