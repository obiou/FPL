% function [lambdaEst,RMS,dNormUpdate,TIRefEst] = SSDUpdateNoMotionDistortion( IRef, TIRef, HRef, M, lambdaEst )
%
% Compute the distortion parameter between a rectified and
% unrectified image using the division model (in image coordinates):
% x_{rect} = \frac{1}{1 + distEst \| x_{dist} \|^2 } x_{dist}  
%
% Input:
% - IRef: unblurred image (from TIRef has been extracted)
% - TIRect: undistorted (rectified) template
% - HRef: homography to obtain the unblurred template
% - distEst: distortion parameter estimate
%
% Output:
...
%
function [distEst,RMS,dNormUpdate,TIRefEst] = SSDUpdateNoMotionDistortion( IRef, TIRect, HRef, distEst )
